...
plot(as.numeric(sort(TA, decreasing = TRUE))[1:let],
  lwd = 2, type = "h", log = "x", col = "magenta", ylab = "Number of reads", xlab = "",
...
plot(as.numeric(sort(TA, decreasing = TRUE))[1:let],
  lwd = 2, type = "h", log = "xy", col = "magenta", ylab = "Number of reads", xlab = "",
  main = "Frequency of GEM barcodes \n (Log xy)", sub = paste0("\n Number of unique GEM barcodes: ", length(unique(names(TA))), "\n Number of total reads in FastQ: ", nrow(mydata))
)
rm(let)
dev.off()
...
write.table(
  x = as.data.frame(sort(TA, decreasing = TRUE)), file = paste0(outdir, GEMraw, "_GapWidth_", GapWidth, ".txt"),
...
#-------------------------------------------------------------------
# Extend BED 500bp from its 3' end -> NOT DONE
#-------------------------------------------------------------------
...
let <- length(sort(TA, decreasing = TRUE))
plot(as.numeric(sort(TA, decreasing = TRUE))[1:let],
  lwd = 2, type = "h", log = "x", col = "orange", ylab = "Number of reads", xlab = "",
  main = "Frequency of GEM barcodes \n (Log x)", sub = paste0("\n Number of unique GEM barcodes: ", length(unique(names(TA))), "\n Number of total reads after Processing: ", nrow(X))
)
plot(as.numeric(sort(TA, decreasing = TRUE))[1:let],
  lwd = 2, type = "h", log = "xy", col = "orange", ylab = "Number of reads", xlab = "",
  main = "Frequency of GEM barcodes \n (Log xy)", sub = paste0("\n Number of unique GEM barcodes: ", length(unique(names(TA))), "\n Number of total reads after Processing: ", nrow(X))
)
rm(let)
dev.off()
...
MergeGenomicRanges <- function(bedData) {
  ...
  grMerged <- reduce(
    x = bedData, drop.empty.ranges = FALSE, min.gapwidth = GapWidth, with.revmap = FALSE, # merge if closer than GapWidth allowed
  ...
  mcols(grMerged)$ReadCount <- bamCount(bampath = BAM, gr = grMerged, mapqual = 0, verbose = FALSE, shift = 0)
...
mergedX2 <- rbindlist(mergedX, use.names = TRUE, fill = FALSE)
...
head(cv)
write.table(
  x = cv, file = paste0(outdir, SampleID, "_GapWidth_", GapWidth, "_ChIA-view.region"), append = FALSE, quote = FALSE, sep = "\t",
